...
from .egg_wrappers import RnnSenderGS, RnnReceiverGS, RnnSenderReinforce, RnnReceiverDeterministic, find_lengths, TransformerSenderReinforce, TransformerReceiverDeterministic, RnnSenderReinforceSPO
...
class EC(nn.Module):
    def __init__(self, args):
        ...
        if args.agent_type == "rnn_reinforce":
            ...
            self.speaker = RnnSenderReinforceSPO(
                Speaker(args),
    ...
    def reinforce_game(self, speaker_input, listener_input, label, aux_input=None, input_dict=None, sender_entropy_coeff=0.1, receiver_entropy_coeff=0.0):
        ...
        message, log_prob_s, entropy_s, step_logits_r = self.speaker(speaker_input, aux_input)
        message_length = find_lengths(message)

        B, msg_len, vocab_size = step_logits_r.size()
        
        step_logits_r = step_logits_r.view(B * msg_len, vocab_size)
        

        rule = torch.argmax(input_dict['rules'].view(B * msg_len, vocab_size), dim=-1)

        loss = F.cross_entropy(step_logits_r, rule, reduction="mean")
        # print(rule.max())
        # exit()
        
        pred = torch.argmax(step_logits_r, dim=-1)
        acc = (pred == rule).detach().float()
        ...
        return loss, {"acc": acc, "pred": pred}, message.detach()
